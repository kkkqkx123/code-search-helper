...
export class NebulaService {
  ...
  private nebulaSessionMonitor: NebulaSessionMonitor;
  ...
  constructor(
    ...
    @inject(TYPES.NebulaSessionMonitor) nebulaSessionMonitor: NebulaSessionMonitor
  ) {
    this.logger = logger;
    this.errorHandler = errorHandler;
    this.nebulaConnection = nebulaConnection;
    this.nebulaSessionMonitor = nebulaSessionMonitor;
  }
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (connected) {
        ...
        this.nebulaSessionMonitor.startMonitoring(2); // Start monitoring with 2-minute intervals for high-concurrency optimization
        this.logger.info('NebulaGraph service initialized successfully with high-concurrency settings');
        return true;
      }
  ...
  async close(): Promise<void> {
    ...
    this.nebulaSessionMonitor.stopMonitoring();
