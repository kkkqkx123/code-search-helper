...
export class AnnotationExtractor extends BaseJavaScriptRelationshipExtractor {
  async extractAnnotationRelationships(
    ...
    // 查找装饰器
    const decorators = this.findNodesByTypes(ast,
    ...
    for (const decorator of decorators) {
      ...
      if (annotationName) {
        relationships.push({
          sourceId: generateDeterministicNodeId(decorator),
          ...
          resolvedAnnotationSymbol: undefined
        });
      }
    }
    ...
    // 查找泛型类型
    const genericTypes = this.findNodesByTypes(ast,
    ...
    for (const genericType of genericTypes) {
      ...
      if (typeName) {
        relationships.push({
          sourceId: generateDeterministicNodeId(genericType),
        ...
        for (const typeArg of typeArguments) {
          relationships.push({
            sourceId: generateDeterministicNodeId(genericType),
            targetId: this.generateNodeId(typeArg, 'annotation', filePath),
            annotationType: 'annotation',
