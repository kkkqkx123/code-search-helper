...
export class ChunkSimilarityUtils {
  ...
  static canMergeChunks(chunk1: CodeChunk, chunk2: CodeChunk, similarityThreshold: number): boolean {
    // 检查相似度
    if (!SimilarityDetector.isSimilar(chunk1.content, chunk2.content, similarityThreshold)) {
      return false;
    }
 ...
 /**
  ...
  static shouldCreateOverlap(
    ...
    for (const existingChunk of existingChunks) {
      ...
      // 检查相似度
      if (SimilarityDetector.isSimilar(newChunk.content, existingChunk.content, similarityThreshold)) {
