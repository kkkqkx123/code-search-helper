...
/**
 * 增强的函数分段器 - 支持更细粒度的函数提取和智能分段
 */
export class EnhancedFunctionSplitter implements FunctionSplitterInterface {
  ...
  constructor(options?: ChunkingOptions) {
    ...
    this.maxFunctionLines = options?.functionSpecificOptions?.maxFunctionLines || 50; // 最大函数行数
    this.minFunctionLines = options?.functionSpecificOptions?.minFunctionLines || 3;  // 最小函数行数
    this.enableSubFunctionExtraction = options?.functionSpecificOptions?.enableSubFunctionExtraction ?? true;
  }
  ...
  extractFunctions(
    ...
    return this.extractEnhancedFunctions(content, ast, language, filePath);
  }
  ...
  async split(
    ...
    if (!this.treeSitterService) {
      throw new Error('TreeSitterService is required for EnhancedFunctionSplitter');
    }
    ...
    if (parseResult && parseResult.success && parseResult.ast) {
      return this.extractEnhancedFunctions(content, parseResult.ast, language, filePath, nodeTracker);
    } else {
      return [];
    }
  }
  ...
  extractEnhancedFunctions(
    ...
    try {
      ...
      this.logger?.warn(`Failed to extract enhanced function chunks: ${error}`);
    }
  ...
  getName(): string {
    return 'EnhancedFunctionSplitter';
  }
