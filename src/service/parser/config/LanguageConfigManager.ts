...
/**
 * 语言配置管理器
 * 整合了原有的 LanguageConfigManager 功能
 */
export class LanguageConfigManager {
  ...
  private initializeLanguageConfigs(): void {
    ...
    this.addLanguageConfig({
      ...
      syntaxRules: [
        {
          name: 'rust_struct',
          description: '处理Rust结构体',
          nodeTypes: ['struct_item'],
          handler: 'handleStructItem',
          priority: 1
        },
        {
          name: 'rust_trait',
          description: '处理Rust特征',
          nodeTypes: ['trait_item'],
          handler: 'handleTraitItem',
          priority: 1
        },
        {
          name: 'rust_macro',
          description: '处理Rust宏',
          nodeTypes: ['macro_item'],
          handler: 'handleMacroItem',
          priority: 2
        }
      ],
      specialRules: [
        {
          name: 'rust_lifetime',
          description: '处理生命周期参数',
          pattern: /'[a-zA-Z_][a-zA-Z0-9_]*/g,
          replacement: 'LIFETIME',
          enabled: true
        },
        {
          name: 'rust_macro_call',
          description: '处理宏调用',
          pattern: /\w+!/g,
  ...
  getLanguageStats(): {
    ...
    totalSyntaxRules: number;
    totalSpecialRules: number;
    ...
    const languageDetails = Array.from(this.configs.values()).map(config => ({
      language: config.language,
      extensions: config.fileExtensions,
      chunkTypes: config.chunkTypes,
      syntaxRules: config.syntaxRules.length,
      specialRules: config.specialRules.length
    }));

    const totalExtensions = languageDetails.reduce((sum, detail) => sum + detail.extensions.length, 0);
    const totalChunkTypes = languageDetails.reduce((sum, detail) => sum + detail.chunkTypes.length, 0);
    const totalSyntaxRules = languageDetails.reduce((sum, detail) => sum + detail.syntaxRules, 0);
    const totalSpecialRules = languageDetails.reduce((sum, detail) => sum + detail.specialRules, 0);

    return {
      totalLanguages: this.configs.size,
      totalExtensions,
      totalChunkTypes,
      totalSyntaxRules,
      totalSpecialRules,
