...
async function main() {
    ...
    if (!testCaseId) {
        console.error('请提供测试用例ID，例如: node test-simple-query.js 003');
        process.exit(1);
    }

    console.log('开始测试查询模式...\n');
    ...
    // 测试不同的查询模式
    await testQuery('c', code, '(declaration) @decl', '测试declaration节点');
    await testQuery('c', code, '(init_declarator) @init', '测试init_declarator节点');
    await testQuery('c', code, '(type_definition) @typedef', '测试type_definition节点');
    await testQuery('c', code, '(type_identifier) @type', '测试type_identifier节点');
    await testQuery('c', code, '(identifier) @id', '测试identifier节点');
