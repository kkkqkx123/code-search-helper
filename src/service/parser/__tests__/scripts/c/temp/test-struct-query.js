...
async function main() {
    ...
    if (!testCaseId) {
        console.error('请提供测试用例ID，例如: node test-struct-query.js 004');
        process.exit(1);
    }

    console.log('开始测试结构体查询模式...\n');
    ...
    // 测试结构体相关的查询模式
    await testQuery('c', code, '(struct_specifier) @struct', '测试struct_specifier节点');
    await testQuery('c', code, '(struct_specifier (type_identifier) @name)', '测试struct_specifier + type_identifier');
    await testQuery('c', code, '(struct_specifier (field_declaration_list) @fields)', '测试struct_specifier + field_declaration_list');
    await testQuery('c', code, '(field_declaration) @field', '测试field_declaration节点');
    await testQuery('c', code, '(field_declaration (field_identifier) @fname)', '测试field_declaration + field_identifier');
