import { injectable, inject } from 'inversify';
import { LoggerService } from '../../../../../utils/LoggerService';
import { TYPES } from '../../../../../types';
import { ISplitStrategy, IStrategyProvider, ChunkingOptions } from '../../../interfaces/ISplitStrategy';
import { UniversalTextSplitter } from '../../../universal/UniversalTextSplitter';

/**
 * 语义策略实现
 * 实现ISplitStrategy接口，使用语义边界进行分段
 */
@injectable()
export class SemanticSplitStrategy implements ISplitStrategy {
  constructor(
    @inject(TYPES.UniversalTextSplitter) private universalTextSplitter?: UniversalTextSplitter,
    @inject(TYPES.LoggerService) private logger?: LoggerService
  ) { }

  async split(
    content: string,
    language: string,
    filePath?: string,
    options?: ChunkingOptions,
    nodeTracker?: any,
    ast?: any
  ) {
    if (!this.universalTextSplitter) {
      this.logger?.warn('UniversalTextSplitter not available, falling back to line strategy');
      // 简单的行分段回退
      return this.fallbackToLineSplitting(content, language, filePath);
    }

    try {
      this.logger?.debug(`Using semantic strategy for ${filePath}`);

      // 使用语义边界分段
      const chunks = await this.universalTextSplitter.chunkBySemanticBoundaries(content, filePath || '', language);

      // 为每个块添加ID
      return chunks.map((chunk, index) => ({
        id: `semantic_${Date.now()}_${index}`,
        ...chunk
      }));
    } catch (error) {
      this.logger?.error(`Semantic strategy failed: ${error}`);
      throw error;
    }
  }

  getName(): string {
    return 'SemanticStrategy';
  }

  getDescription(): string {
    return 'Uses semantic boundary detection for intelligent code splitting';
  }

  supportsLanguage(language: string): boolean {
    // 语义策略支持所有语言
    return true;
  }

  getPriority(): number {
    return 3; // 中等优先级
  }

  /**
   * 简单的行分段回退方法
   */
  private fallbackToLineSplitting(content: string, language: string, filePath?: string): any[] {
    const lines = content.split('\n');
    const chunks = [];
    const maxLines = 50; // 每个块最多50行
    
    for (let i = 0; i < lines.length; i += maxLines) {
      const chunkLines = lines.slice(i, i + maxLines);
      const chunkContent = chunkLines.join('\n');
      
      chunks.push({
        id: `semantic_fallback_${Date.now()}_${Math.floor(i / maxLines)}`,
        content: chunkContent,
        metadata: {
          startLine: i + 1,
          endLine: Math.min(i + maxLines, lines.length),
          language: language,
          filePath: filePath,
          type: 'semantic' as const
        }
      });
    }
    
    return chunks;
  }
}

/**
 * 精细语义策略实现
 * 使用更精细的语义分段参数
 */
@injectable()
export class SemanticFineSplitStrategy implements ISplitStrategy {
  constructor(
    @inject(TYPES.UniversalTextSplitter) private universalTextSplitter?: UniversalTextSplitter,
    @inject(TYPES.LoggerService) private logger?: LoggerService
  ) { }

  async split(
    content: string,
    language: string,
    filePath?: string,
    options?: ChunkingOptions,
    nodeTracker?: any,
    ast?: any
  ) {
    if (!this.universalTextSplitter) {
      this.logger?.warn('UniversalTextSplitter not available, falling back to line strategy');
      throw new Error('UniversalTextSplitter not available');
    }

    this.logger?.debug(`Using fine semantic strategy for ${filePath}`);

    // 保存原始选项
    const originalOptions = {
      maxChunkSize: this.universalTextSplitter.getOptions?.()?.maxChunkSize || 2000,
      overlapSize: this.universalTextSplitter.getOptions?.()?.overlapSize || 20,
      maxLinesPerChunk: this.universalTextSplitter.getOptions?.()?.maxLinesPerChunk || 50
    };

    try {
      // 根据内容大小调整重叠大小，小文件使用更小的重叠
      const contentLines = content.split('\n').length;
      const adjustedOverlapSize = Math.min(50, Math.max(20, contentLines * 2)); // 每行约2-50字符重叠

      // 临时设置更精细的分段参数
      this.universalTextSplitter.setOptions({
        maxChunkSize: 800, // 从2000降低到800
        maxLinesPerChunk: 20, // 从50降低到20
        overlapSize: adjustedOverlapSize,   // 动态调整重叠大小
        enableSemanticDetection: true
      });

      const chunks = await this.universalTextSplitter.chunkBySemanticBoundaries(content, filePath || '', language);

      this.logger?.debug(`Semantic fine strategy completed, generated ${chunks.length} chunks`);

      // 为每个块添加ID
      return chunks.map((chunk, index) => ({
        id: `semantic_fine_${Date.now()}_${index}`,
        ...chunk
      }));

    } catch (error) {
      this.logger?.error(`Semantic fine strategy failed: ${error}`);
      throw error;
    } finally {
      // 恢复原始选项
      try {
        this.universalTextSplitter.setOptions({
          ...originalOptions,
          enableSemanticDetection: true
        });
      } catch (restoreError) {
        this.logger?.warn(`Failed to restore original text splitter options: ${restoreError}`);
      }
    }
  }

  getName(): string {
    return 'SemanticFineStrategy';
  }

  getDescription(): string {
    return 'Uses fine-grained semantic segmentation with adjusted parameters';
  }

  supportsLanguage(language: string): boolean {
    // 精细语义策略支持所有语言
    return true;
  }

  getPriority(): number {
    return 2; // 高优先级
  }
}

/**
 * 语义策略提供者
 */
@injectable()
export class SemanticStrategyProvider implements IStrategyProvider {
  constructor(
    @inject(TYPES.UniversalTextSplitter) private universalTextSplitter?: UniversalTextSplitter,
    @inject(TYPES.LoggerService) private logger?: LoggerService
  ) {}

  getName(): string {
    return 'universal_semantic';
  }

  createStrategy(options?: ChunkingOptions): ISplitStrategy {
    return new SemanticSplitStrategy(
      options?.universalTextSplitter || this.universalTextSplitter,
      this.logger
    );
  }

  getDependencies(): string[] {
    return ['UniversalTextSplitter'];
  }

  supportsLanguage(language: string): boolean {
    return true; // 支持所有语言
  }

  getPriority(): number {
    return 3; // 中等优先级
  }

  getDescription(): string {
    return 'Provides semantic boundary-based code splitting';
  }
}

/**
 * 精细语义策略提供者
 */
@injectable()
export class SemanticFineStrategyProvider implements IStrategyProvider {
  constructor(
    @inject(TYPES.UniversalTextSplitter) private universalTextSplitter?: UniversalTextSplitter,
    @inject(TYPES.LoggerService) private logger?: LoggerService
  ) {}

  getName(): string {
    return 'universal_semantic_fine';
  }

  createStrategy(options?: ChunkingOptions): ISplitStrategy {
    return new SemanticFineSplitStrategy(
      options?.universalTextSplitter || this.universalTextSplitter,
      this.logger
    );
  }

  getDependencies(): string[] {
    return ['UniversalTextSplitter'];
  }

  supportsLanguage(language: string): boolean {
    return true; // 支持所有语言
  }

  getPriority(): number {
    return 2; // 高优先级
  }

  getDescription(): string {
    return 'Provides fine-grained semantic boundary-based code splitting';
  }
}