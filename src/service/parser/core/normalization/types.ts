...
export interface StandardizedQueryResult {
  ...
  nodeId: string;
  /** 结构类型 */
  type: 'function' | 'class' | 'method' | 'import' | 'variable' | 'interface' | 'type' | 'export' | 'control-flow' | 'expression' | 'config-item' | 'section' | 'key' | 'value' | 'array' | 'table' | 'dependency' | 'type-def' | 'call' | 'data-flow' | 'parameter-flow' | 'inheritance' | 'implements' | 'concurrency' | 'lifecycle' | 'semantic' | 'union' | 'enum' | 'annotation' | 'comment';
