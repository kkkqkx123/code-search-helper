...
/**
 * Graph模块批处理配置管理器
 * 负责根据文件特征和系统资源动态计算最优批处理配置
 */
@injectable()
export class GraphBatchConfigManager {
  constructor(
   ...
   * 获取批处理配置摘要
   */
  async getBatchConfigSummary(files: string[]): Promise<{
    batchSize: number;
    concurrency: number;
    estimatedBatches: number;
    reasoning: string[];
  }> {
    const batchSize = await this.calculateOptimalBatchSize(files);
    const concurrency = await this.calculateOptimalConcurrency(files);
    const estimatedBatches = Math.ceil(files.length / batchSize);

    const reasoning: string[] = [];

    // 添加配置决策的推理说明
    const avgFileSize = await this.calculateAverageFileSize(files);
    if (await avgFileSize > 100 * 1024) {
      reasoning.push(`文件较大 (平均${Math.round(avgFileSize / 1024)}KB)，使用较小批次`);
    }

    const fileTypes = this.analyzeFileTypes(files);
    if (fileTypes.has('typescript') || fileTypes.has('java')) {
      reasoning.push('包含复杂语言文件，减少批次大小以提高处理质量');
    }

    const systemLoad = this.getSystemLoad();
    if (systemLoad > 0.6) {
      reasoning.push(`系统负载较高 (${Math.round(systemLoad * 100)}%)，降低并发数`);
    }
