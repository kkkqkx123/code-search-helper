...
async function analyzeVueAST(Vue) {
    ...
    try {
        ...
        // 在tree-sitter 0.25.0中，我们需要直接使用语言对象
        // 但首先检查它是否已经是正确的格式
        try {
            ...
            console.log('直接设置Vue语言失败，尝试其他方法...');
            ...
            try {
                ...
                const fs = require('fs');
                const path = require('path');
                const bindingPath = path.join(__dirname, 'node_modules', 'tree-sitter-vue', 'build', 'Release', 'tree_sitter_vue_binding.node');
                
                if (fs.existsSync(bindingPath)) {
                    console.log('找到Vue二进制绑定文件，尝试直接加载...');
                    const binding = require(bindingPath);
                    
                    // 创建一个新的语言对象
                    const language = Object.create(Vue);
                    language.constructor = binding.Language || Vue.constructor;
                    
                    parser.setLanguage(language);
                    const tree = parser.parse(vueCode);
                    console.log('Vue AST结构 (使用二进制绑定):');
                    console.log(tree.rootNode.toString());
                } else {
                    throw new Error('Vue二进制绑定文件不存在');
                }
            } catch (bindingError) {
                console.log('Vue二进制绑定加载失败:', bindingError.message);
                
                // 最后的备用方案：使用HTML解析器解析Vue模板部分
                try {
                    console.log('尝试使用HTML解析器解析Vue模板部分...');
                    ...
                    if (templateMatch) {
                        ...
                        const tree = parser.parse(templateCode);
                        console.log('Vue模板AST结构 (使用HTML解析器):');
                        console.log(tree.rootNode.toString());
                    } else {
                        throw new Error('无法找到Vue模板部分');
                    }
                } catch (htmlError) {
                    console.log('HTML解析器也失败:', htmlError.message);
                    throw setError;
                }
            }
        }
